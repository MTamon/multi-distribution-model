...
from nemo.collections.asr.metrics.wer import CTCDecodingConfig, word_error_rate_detail
from nemo.collections.asr.models import EncDecCTCModel, EncDecHybridRNNTCTCModel
from nemo.collections.asr.modules.conformer_encoder import ConformerChangeConfig
from nemo.collections.asr.parts.utils.eval_utils import clean_label
...
def cal_write_wer(
    ...
    with open(pred_manifest, "r") as fp:
        for line in fp:
            ...
            sample[
                eval_metric
            ] = wer  # evaluatin metric, could be word error rate of character error rate
    ...
    (
        ...
        total_del_rate,
        total_sub_rate,
    ) = word_error_rate_detail(hypotheses=hyps, references=refs, use_cer=use_cer)
...
def write_transcription(
    transcriptions: Union[
        List[rnnt_utils.Hypothesis], List[List[rnnt_utils.Hypothesis]]
    ],
    cfg: DictConfig,
    model_name: str,
    ...
    with open(cfg.output_filename, "w", encoding="utf-8") as f:
        if cfg.audio_dir is not None:
            for idx, transcription in enumerate(
                ...
                f.write(json.dumps(item, indent=None, ensure_ascii=False) + "\n")
        else:
...
def main(cfg: TranscriptionConfig) -> Union[TranscriptionConfig, List[Hypothesis]]:
    ...
    if cfg.eval_config_yaml:
        ...
        logging.info(
            f"Will apply on-the-fly augmentation on samples during transcription: {augmentor} "
        )
    ...
    if hasattr(asr_model, "change_decoding_strategy"):
        if cfg.decoder_type is not None:
            ...
            decoding_cfg.compute_timestamps = (
                cfg.compute_timestamps
            )  # both ctc and rnnt support it
    ...
    with open_dict(cfg):
        if isinstance(asr_model, EncDecCTCModel) or (
            isinstance(asr_model, EncDecHybridRNNTCTCModel)
            and cfg.decoder_type == "ctc"
        ):
    ...
    if (
        not cfg.return_transcriptions
        and not cfg.overwrite_transcripts

...
Model1 = Net(Input_size, Hidden1_size, Hidden2_size, Output_size).to(Device)
print(Model1)
Model2 = Net(Input_size, Hidden1_size, Hidden2_size, Output_size, distribution_num=2).to(Device)
print(Model2)

# モデルのロード
Model2.load_state_dict(torch.load("Model2Dist.pth"))
...
def visualize_test_result(model: Net, test_loader, device="cpu", forcus_label=6):
    ...
    with torch.no_grad():  # 勾配計算の無効化
        for images, labels in test_loader:
            ...
            for i in range(10):
                print(bf[i].view(2, 10)[:, forcus_label], int(predicted[i]), int(max_idx[i]))
            input()
            ...
            for i, pred in enumerate(predicted):
                ...
                if completes == model.distribution_num:
                    assert len(images_list[0]) == 100 and len(images_list[1]) == 100
                    break
...
# # モデルの保存
# torch.save(Model2.state_dict(), "Model2Dist.pth")
...
# plt.legend()
# plt.grid()
# plt.savefig("2_distribution_minist1vs2.png")
# plt.close()
